#!/usr/bin/env python3
"""Migrate a repo's note + snapshot status vocabulary to the ADR-0008 taxonomy.

ADR-0008 ("States must earn their keep", as amended 2026-07-25) collapsed the
project-os status taxonomy from 64 declared values to 53. This script rewrites
the values that changed, in note frontmatter and in the matching SNAPSHOT.yaml
entries, so the two never disagree mid-migration.
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Migrate status vocabulary to the ADR-0008 taxonomy.")
